    ...
    <div>
      ...
      {apiKey && (
        <Card className="border-violet-300 bg-violet-50 mb-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-zinc-800">AI Caption Generator</h3>
          </div>
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 text-sm text-zinc-700 cursor-pointer">
                <input
                  ...
                  className="rounded border-zinc-600 bg-zinc-100 text-violet-500 focus:ring-violet-500"
                />
                Include Emojis
              </label>
              <label className="flex items-center gap-2 text-sm text-zinc-700 cursor-pointer">
          ...
          {aiPreview && (
            <div className="mt-4 p-3 bg-surface-elevated rounded-lg border border-zinc-300">
              <div className="flex items-center gap-2 mb-2">
                ...
                <Badge className="bg-zinc-200 text-zinc-700">{aiPreview.category}</Badge>
              </div>
      ...
      {templates.length === 0 && !apiKey ? (
        ...
        <>
          ...
          {templates.length === 0 ? (
            <p className="text-sm text-zinc-400 text-center py-8">No saved templates yet. Use AI above or create one manually.</p>
          ) : (
            <div className="space-y-3">
              {templates.map(t => (
                <Card key={t.id}>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-sm font-medium text-zinc-800">{t.name}</h3>
                        <PlatformBadge platform={t.platform} />
                        {t.category && <Badge className="bg-zinc-200 text-zinc-700">{t.category}</Badge>}
                      </div>
                    </div>
                    <div className="flex gap-1">
                      <button onClick={() => copyTemplate(t.id, t.template)} className="p-1.5 text-zinc-400 hover:text-violet-600 transition-colors">
                        {copiedId === t.id ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
                      </button>
                      <button onClick={() => deleteTemplate(t.id)} className="p-1.5 text-zinc-400 hover:text-red-600 transition-colors">
