...
export function IdeaCard({ idea, onClick, onToggleFavorite }: IdeaCardProps) {
  return (
    <div
      onClick={onClick}
      className="bg-surface-card border border-zinc-200 rounded-lg p-4 cursor-pointer hover:border-zinc-300 transition-colors"
    >
      <div className="flex items-start justify-between mb-2">
        ...
        >
          <Star className={`w-4 h-4 ${idea.isFavorite ? 'fill-yellow-400 text-yellow-600' : ''}`} />
        </button>
      </div>
      <h3 className="text-sm font-medium text-zinc-800 mb-1">{idea.title}</h3>
